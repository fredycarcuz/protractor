...
var FillIncidents = function(driverNumber,incidentTypeList){
    var incidentsBtn = element(by.id('incident_yes_d'));
    ...
    for (var i = 0; i < incidentTypeList.incidents.length; i++) {
        ...
        //Choose driver
        element(by.id('driverName_' + incidentNumber + '_d0')).$('[value="'+ driverNumber +'"]').click(); browser.sleep(400);
        if (incidentType==='Ticket'){
            element(by.id('incidentType_'+ incidentNumber +'_d0')).$('[value="'+ incidentType +'"]').click(); browser.sleep(400);
        ...
        if (incidentType==='Ticket' || incidentType==='DUI') {
        ...
        if (incidentNumber < incidentTypeList.incidents.length){
            element(by.css('.addIncid')).click(); //Add Incident
        }
    }
};
